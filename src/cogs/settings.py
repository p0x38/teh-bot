...
class SettingsCog(commands.Cog):
    ...
    async def guild_language(self, ctx: commands.Context, lang: str):
        if not ctx.guild:
    ...
    async def set_personality(self, ctx: commands.Context, style: str):
        if not ctx.guild:
            locale_str = str(ctx.interaction.locale) if ctx.interaction else "en"
            return await ctx.reply(
                self.bot.translator.t(locale_str, "errors-settings-guild-only")
    ...
    async def user_tz(self, ctx: commands.Context, tz: str):
